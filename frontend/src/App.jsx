...
function App() {
  ...
  function NewNoteModal() {
    ...
    return (
      <>
        ...
        <IconButton
          sx={{ position: 'fixed', zIndex: 10, bottom: 60, right: 30, display: { sm: 'none', xs: 'inline-flex' } }}
          size="lg"
          variant="solid"
          onClick={() => setModalopen(true)}
          color="primary">
  ...
  return (
    <CssVarsProvider disableTransitionOnChange>
      ...
      <Box sx={{ margin: 2, marginBottom: 4, paddingX: { sm: 1, md: 4 }, display: 'flex', justifyContent: 'space-between' }}>
        <NewNoteModal />
        <Input
          type="text"
          sx={{ marginX: 2 }}
      ...
      <Grid spacing={2} sx={{ flexGrow: 1, padding: { sm: 1, md: 4 }, marginBottom: 10 }}>

        {
          notes.length > 0 ? (<Grid md={10} container
            spacing={{ xs: 2, md: 3 }}
            columns={{ xs: 4, sm: 8, md: 12 }}
            sx={{ flexGrow: 1, paddingX: 4 }} gap={2} >

            {
              notes.map((item) => (
                <NoteCard key={item._id} title={item.title} description={item.description} updated={item.updatedAt} id={item._id} getNotes={getNotes} />
              ))
            }
          </Grid>) :
